...
float AverageMonthlyTemperature(struct sensor *temperature_information, int N, int M)
{
    ...
    int counter = 0;
    for (int i = 0; i < N; i++)
        if (temperature_information[i].month == M)
        {
...
{
    int min_temp = temperature_information[0].temp;
    for (int i = 1; i < N; i++)
        if (temperature_information[i].month == M && min_temp > temperature_information[i].temp)
...
{
    int max_temp = temperature_information[0].temp;
    for (int i = 1; i < N; i++)
        if (temperature_information[i].month == M && max_temp < temperature_information[i].temp)
